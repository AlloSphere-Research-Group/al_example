 ...
 public:
  ...
  virtual void operator()() override {
    while (running) {
      ...
      // or functions
      amp.set(newAmpDb);
      cout << "Setting through C++: Frequency " << newFreq << " Amplitude "
           << newAmpDb << endl;
...
// Parameter::setProcessingCallback()
...
int main(int argc, char* argv[]) {
  ...
  paramServer.addListener("127.0.0.1", 13560);  // Try listening for OSC on this
                                                // port to check that values are
                                                // being forwarded
