...
enum { RING_LAYOUT, THREE_D_LAYOUT, ALLO_LAYOUT, TEST_LAYOUT };
...
inline void perSample(AudioIOData& io) {
  ...
  for (int j = 0; j < numFrames; j++) {
    ...
    for (int i = 0; i < NUM_SOURCES; i++) {
      ...
      float tta =
          sec * speedMult * 2.0 * M_PI + (2.0 * M_PI / NUM_SOURCES) * (i + 1.0);
...
class MyApp : public App {
 ...
 public:
  MyApp(std::vector<SpeakerTriple> sts)
      : mFont("allocore/share/fonts/VeraMoIt.ttf", 20) {
    mLight.pos(0, 6, 0);
...
int main(int argc, char* argv[]) {
  ...
  if (layout == THREE_D_LAYOUT) {
    ...
    speakerLayout.addSpeaker(Speaker(
        5, 0, -80,
        rad / 2));  // This channel will be turned into a phantom speaker
  }
