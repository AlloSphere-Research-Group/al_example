...
void audioCB(AudioIOData &io) {
  ...
  for (int i = 0; i < 1; i++) {
    for (int j = 0; j < framesRead; j++) {
      ...
      int chan = 0;
      ambiChans[j + chan * AUDIO_BLOCK_SIZE] =
          (ud->read_buffer[j * 4 + chan] * ud->gain);
...
int main(int argc, char *argv[]) {
  ...
  //    std::string filename =
  //    "Ambisonics/Ambisonia/PWH_Purcell-Your_hay_it_is_mowed.amb";
  //    std::string filename =
  //    "Ambisonics/Ambisonia/Royer-Werni-Töpp_Improvisation_#3-CoreSound_TetraMic-24bit-48k.amb";
  //    std::string filename =
  //    "Ambisonics/Ambisonia/cv_Badlands_Day_Insects.amb";
  std::string filename = "Ambisonics/Bees/T08.WAV";
  //	std::string filename =
  //"Ambisonics/Ambisonia/The_Kites-Conference_of_the_Birds-CoreSound_TetraMic-24bit-48k.amb";
  //	std::string filename =
  //"Ambisonics/Ambisonia/The_Kites-Conference_of_the_Birds-Soundfield_ST450-24bit-48k.amb";
