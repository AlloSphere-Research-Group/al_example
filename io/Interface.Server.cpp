...
This is a demo integrating Allocore with Interface.Server (
https://github.com/charlieroberts/interface.server.2 )
...
6. Run "npm install interface.server.keyboard" This will install keyboard
support
7. To run the server, "node ."

Then run this demo file using run.sh. If you press the 'b' key in the node.js
terminal window, you should see
...
int main() {
  ...
  /*
  * /handshake message tells Interface.Server to load a specific application
* description, located in a folder named 'applications'
* found in the Interface.Server repo. In this case we use the application
* "simple", which is the most basic example that
* comes with the Interface.Server repo.
  */
  ...
  printf(
      "press return to quit the application and disconnect from "
      "Interface.Server.\n");
  getchar();
