...
class ZeroconfNotifier : public zero::Client {
 public:
  ...
  virtual void onServiceResolved(const std::string& name,
                                 const std::string& host_name, uint16_t port,
                                 const std::string& address) {
    printf(
        "Zeroconf: resolved service '%s' on host '%s' on port %u at address "
        "'%s'\n",
...
// a Client can browse and report available services on the network for a given
// service type:
