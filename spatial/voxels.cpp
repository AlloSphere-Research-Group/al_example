...
Description:
This shows basic usage of the Voxel class which represents a 3-dimensional
vector with physical size units
...
// function used to initialize array data (from
// allocore/examples/graphics/texture3d.cpp)
...
int main() {
  ...
  std::cout << "One voxel is " << v.printVoxWidth(0) << " by "
            << v.printVoxWidth(1) << " by " << v.printVoxWidth(2)
            << " so its volume is "
            << (v.getVoxWidth(0) * v.getVoxWidth(1) * v.getVoxWidth(2))
            << " cubic " << v.printUnits() << std::endl;

  std::cout << "the upper left 12x12 of the 7th plane in Z looks like this: "
            << std::endl;
