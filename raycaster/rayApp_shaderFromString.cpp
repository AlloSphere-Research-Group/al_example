...
struct MyApp : public RayApp {
  ...
  virtual void loadShaders() override {
    ShaderProgram* s =
        mShaderManager.addShaderString("toroid", vertexCode(), fragmentCode());
    s->begin();
  ...
  virtual bool onKeyDown(const Keyboard& k) override {
    if (k.ctrl()) {
      switch (k.key()) {
        case '1':
          mOmni.mode(RayStereo::MONO);
          return false;
        case '2':
          mOmni.mode(RayStereo::SEQUENTIAL);
          return false;
        case '3':
          mOmni.mode(RayStereo::ACTIVE);
          return false;
        case '4':
          mOmni.mode(RayStereo::DUAL);
          return false;
        case '5':
          mOmni.mode(RayStereo::ANAGLYPH);
          return false;
        case '6':
          mOmni.mode(RayStereo::LEFT_EYE);
          return false;
        case '7':
          mOmni.mode(RayStereo::RIGHT_EYE);
          return false;
      }
    } else {
      if (k.key() == '1') {
