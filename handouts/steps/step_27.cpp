// Make visual motion of balls follow glissandi
...
struct Agent {
  ...
  Color color;
  Vec3f position;
  float currentFreq;  // Kept up to date by onSound()
  float previousFreq; // My frequency as of the previous call to onAnimate()
  ...
  Agent() {
    ...
    //  2. Gradually decay as long as necessary for overall duration (sustain segment)
#define SUSTAIN_SEGMENT 1 // The segment from level 1 to level 2 
    ...
    ampEnv.lengths(quick*uniform(.9,1.1), 
                   0 /*sustain*/, 
                   quick*uniform(.9,1.1), 
                   quick*uniform(.9,1.1) );
    ...
    std::cout << "Calling Agent constructor; song_length is " << 
      song_length << std::endl;
    ...
    // Initialize current and previous freq (used by onAnimate)
    previousFreq = currentFreq = frequencies[0];
  ...
  void printSong() {
    ...
    for (int i = 0; i<song_length; ++i) {
      cout << " note " << i << ": " << frequencies[i] << " Hz, " << durations[i] << " seconds, "
           << (gliss[i] ? "glissando" : "no glissando") << endl;
    }
  }
  ...
  void printEnvelope() {
    for (int i = 0; i<ampEnv.size()-1; i++) {
      std::cout<<"level["<<i<<"]="<<ampEnv.levels()[i]<<
        ",length["<<i<<"]="<<ampEnv.lengths()[i]<<",";
    }
    int lasti = ampEnv.size()-1;
    std::cout<<"level["<<lasti<<"]="<<ampEnv.lengths()[lasti]<<std::endl;
  }


  void onAnimate(double dt) {
    float interval = currentFreq / previousFreq;
    float intervalInHalfSteps = log2(interval) / log2(pow(2.0, 1.0/12.0));
    // cout << "Interval " << interval << " so halfsteps = " << intervalInHalfSteps << endl;

    position += Vec3f(0, intervalInHalfSteps * 0.1, 0);
  ...
  float onSound() {
    if (tmr()) {
      ...
      if (gliss[current_note]) {
        // Glissando: during this note we slide from the old freq to the new freq
        freqEnv.levels(oldFreq,newFreq);
      } else {
        // No glissando:  during this note we "slide" from the new freq to the new freq,
        // i.e., the envelope will output newFreq as a constant value, so our note will
        // have a steady pitch.
      ...
      if (ampEnv.lengths()[SUSTAIN_SEGMENT] < 0.) {
        // We want a note shorter than the "quick" three segments allow, so permanently
        // make them even quicker
        float *l = ampEnv.lengths();
        float oldMinDuration = l[0]+l[2]+l[3];
        float s /* slimming factor */ = durations[current_note] / oldMinDuration;
        s *= 0.99;  // A little bit smaller just to be safe
        ampEnv.lengths(l[0]*s, 0 /*sustain*/, l[2]*s, l[3]*s);

        std::cout << "Slimmed by " << s << " to get a shorter envelope for " << whichAgentAmI << ":" << std::endl;
        printEnvelope();
    ...
    // Apply frequency envelope by using its next sample to set the frequency of my sine:
    currentFreq = freqEnv();
    sine.freq(currentFreq);

    // Apply amplitude envelope by using its next sample to scale the output of my sine:
    return sine()*ampEnv();
  }
