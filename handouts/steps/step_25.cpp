// Make vertical motion of balls follow the pitch intervals of the melody
...
#define SUSTAIN_SEGMENT 1 // The segment from level 1 to level 2 
  ...
  float onSound() {
    if (tmr()) {
      ...
      float oldFreq = sine.freq();
      sine.freq(frequencies[current_note]);
      float newFreq = sine.freq();
      float interval = newFreq/oldFreq;

      float intervalInHalfSteps = log2(interval) / log2(pow(2.0, 1.0/12.0));
      cout << "Freq. ratio of " << interval << " = " << intervalInHalfSteps << " half steps" << endl;
