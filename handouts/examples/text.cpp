...
public:
	...
	MyApp()
	:	font1("handouts/media/Avenir-Medium.otf", 40)
	{
    nav().pos(0, 0, 10);
    light.pos(0, 0, 10);
		addSphere(m);
    ...
    Vec3d forward = Vec3d(nav().pos() - textPos).normalize(); // vector from source to destination
    Quatd rot = Quatd::getBillboardRotation(forward, nav().uu()); // pass forward vector and up vector
    g.rotate(rot);
     ...
     */
		...
		g.translate(int(W/2 - font1.width(str)/2), int(H/2 - font1.size()/2));
