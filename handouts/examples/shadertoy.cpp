...
// - Look in handouts/shaders for other examples
//
...
struct MyApp : App {
  ...
  void loadShaders() {
    Shaders::paths().addSearchPath("handouts/shaders", true);
    Shaders::vertLibCode = "#version 120\n";
    Shaders::load("live", "basic.vert", "color.frag");
    // Shaders::load("live","basic.vert","waves.frag");
    // Shaders::load("live","basic.vert","tweets.frag");
  ...
  void onDraw(Graphics& g) {
    ...
    if (!inited || Shaders::poll()) loadShaders();
    ...
    // bind shader program
    ShaderProgram* program = Shaders::get("live");
    program->begin();
