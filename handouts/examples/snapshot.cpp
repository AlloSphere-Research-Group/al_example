...
void snapshot(string fileName, int width, int height) {
  Image image;
  image.resize<uint8_t>(width, height, Image::RGB);
  glReadPixels(0, 0, width, height, GL_RGB, GL_UNSIGNED_BYTE,
               image.pixels<uint8_t>());
  if (glGetError() != GL_NO_ERROR)
    cerr << "OpenGL error after glReadPixels. Make sure you are calling "
            "snapshot() from within onDraw()"
         << endl;
  image.save(fileName);
}
...
struct AlloApp : App {
  ...
  virtual void onKeyDown(const ViewpointWindow&, const Keyboard& k) {
    ...
    if (k.key() == 'n') shouldTakeSnapshot = true;
  }
};
