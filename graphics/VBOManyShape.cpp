 ...
 public:
  ...
  void scatterShapes() {
    for (int i = 0; i < numShapes; ++i) {
      ...
      Color randc =
          Color(rnd::uniform(), rnd::uniform(), rnd::uniform(0.75, 1.0));
      Colori randci = Colori(rnd::uniform() * 255, rnd::uniform() * 255,
                             rnd::uniform(0.75, 1.0) * 255);
      for (int i = 0; i < Nv; ++i) {
  ...
  void onKeyDown(const Keyboard& k) {
    if (k.key() == 'm') {
      ...
      if (useVBO)
        ...
        cout << "Streaming every frame" << endl;
    }
