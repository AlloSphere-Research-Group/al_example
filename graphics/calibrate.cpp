...
We know azimuth/elevation to the corners ABCD. From this we can derive their
spatial locations.
Assume the allosphere is a unit sphere, then the result is simple polar to
cartesian.
        This can be refined by modeling the sphere properly as two unit
hemispheres joined by a cylinder of some width W.
        The adjustment W can be partially verified by ensuring that the normals
of ABC and BCD are equal.
Calculate point Q as the center of ABCD (B.x-A.x, D.y-A.y)
        Again, two ways to calculate Q, which can be used to minimize error in
the sphere model.
...
Vertex locations v must be projected into the view of the projector, which is a
matrix multiplication.
We only care about the XY components of the result, but these must be scaled
such that A is -1,-1 and C is 1, 1.
...
Not sure if v should be normalized or not, or whether this makes any difference
at all.
...
struct MyWindow : Window {
  ...
  bool onFrame() {
    ...
    if (bGrid) {
      ...
      gl.loadMatrix(
          Matrix4d::lookAt(Vec3d(0, 0, -2.5), Vec3d(0, 0, 0), Vec3d(0, 1, 0)));
