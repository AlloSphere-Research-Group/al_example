...
struct MyWindow : public Window {
  ...
  bool onFrame() {
    ...
    iso.generate((char *)tex.array().data.ptr, 32, 32, 32, 1. / 32, 1. / 32,
                 1. / 32);
...
MRCHeader &mrcParse(const char *data, Array &array) {
  ...
  bool swapped =
      (header.nx <= 0 || header.ny <= 0 || header.nz <= 0 ||
       ...
       header.mapx < 0 || header.mapx > 4 || header.mapy < 0 ||
       header.mapy > 4 || header.mapz < 0 || header.mapz > 4);
